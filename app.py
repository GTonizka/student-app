...
try:
    ...
    with tab1:
        if not students_df.empty:
            search_input = st.text_input("🔍 지도가 필요한 학생 이름 입력", placeholder="이름 입력 후 엔터 (예: 홍길동)", key="g_search_name")
            
            if search_input:
                matched_df = students_df[students_df['이름'] == search_input]
                
                if matched_df.empty:
                    st.warning(f"'{search_input}' 학생을 찾을 수 없습니다. 이름을 정확히 입력해 주세요.")
                else:
                    is_selected = False
                    
                    if len(matched_df) == 1:
                        target_stu = matched_df.iloc[0]
                        is_selected = True
                    else:
                        ...
                        search_grade = str(sel_option.split("학년")[0].strip())
                        search_class = str(sel_option.split("학년 ")[1].split("반")[0].strip())
                        target_stu = matched_df[
                            (matched_df['학년'].astype(str) == search_grade) & 
                            (matched_df['반'].astype(str) == search_class)
                        ].iloc[0]
                        is_selected = True

                    if is_selected:
                        search_guide = target_stu['이름']
                        st.success(f"📍 {target_stu['학년']}학년 {target_stu['반']}반 {target_stu['번호']}번 {target_stu['이름']} (상태: {target_stu['학적상태']})")
